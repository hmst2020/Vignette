...
X<- t(t(std.qn[,ka]*sqrt(rw))*sqrt(cw))              # 慣量
#X<- std.qn[,ka]              # 慣量
print(t(X)%*%X)             # 用來特徵分解  
...
U<- ifelse(nrow(X)<=ncol(X),list(SVD$v),list(SVD$u))[[1]]      # 左奇異矩陣
...
t(t(std.qn[,ka]%*%Q)*(1/sqrt(lambda)))          # 個體座標
t(t(t(std.qn[,ka])%*%P)/nrow(std.qn[,ka]))/sqrt(lambda)    # 變數座標
t(t(std.qn[,ka]%*%pca$var$coord)*(1/sqrt(lambda)))         # 個體座標
t(t(t(std.qn[,ka])%*%(pca$ind$coord)/nrow(std.qn[,ka]))/sqrt(lambda))    # 變數座標
#####補充個體主成分#####
...
all(round(sup.ind.coord,7)==round(pca$ind.sup$coord,7))  #印證公式
#####補充變數主成分#####
